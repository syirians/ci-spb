...
class Produk extends SB_Controller 
{
	...
	function index() 
	{
		...
		$sort = (!is_null($this->input->get('sort', true)) ? $this->input->get('sort', true) : 'idProduk'); 
		...
		$this->data['content'] = $this->load->view('produk/index',$this->data, true );
	...
	function show( $id = null) 
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('t_produk'); 
		}
		
		$this->data['id'] = $id;
		$this->data['content'] =  $this->load->view('produk/view', $this->data ,true);	  
		$this->load->view('layouts/main',$this->data);
	}
	...
	function add( $id = null ) 
	{
		...
		$this->data['id'] = $id;
		$this->data['content'] = $this->load->view('produk/form',$this->data, true );		
	  	$this->load->view('layouts/main', $this->data );
	...
	function save($id = null) {
		...
		if( !empty($rules) && $this->form_validation->run()){
			...
			$result = $this->db->query("SELECT * FROM t_produk where idProduk='$data[idProduk]'")->result_array();
			$row = $this->db->query("SELECT * FROM t_produk where idProduk='$data[idProduk]'")->row();
			...
			if ($id != NULL) {
				...
				$dataInput=array(
					...
					'kapasitasTank'=> $data['kapasitasTank'],
				);
				...
				$ID = $this->model->insertRow($dataInput, NULL);
			}
			// Input logs
